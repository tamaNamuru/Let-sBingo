	...
	if(urow != -1) {
		...
		num.onclick = () => {
			...
			if(max == 4 && !reachflag){
                ...
                document.getElementById("reachpng").style.display = "inline";
				setTimeout(() => {
                    document.getElementById("reachpng").style.display = "none";
                },5000);
			}else if(max == 5){
                waitflag = true;
				guest.emit('bingo', () => { waitflag = false;});
                document.getElementById("bingopng").style.display = "inline";
				setTimeout(() => {
                    document.getElementById("bingopng").style.display = "none";
                },5000);
			}
		}
	}
